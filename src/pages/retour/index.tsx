import { addRetour } from '@/lib/helpers'
...
export default function  Index () {
    ...
    if(Object.keys(formData).length == 0) {
      ...
      const response =  await addRetour(formData);
      if(response) {toast.info('Nous allons vous contacter le plus tôt possible !',{
        position: toast.POSITION.TOP_RIGHT,
        theme: "colored"
      });} else if(response.error) {
        {toast.error(`Erreur nous countacter , Merci :  ${response.error}`,{
            position: toast.POSITION.TOP_RIGHT,
            theme: "colored"});
      }}}
    return <>
